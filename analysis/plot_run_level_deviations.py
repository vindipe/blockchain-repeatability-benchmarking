...
def render_outcome_table(counts: pd.DataFrame) -> str:
    ...
    lines = [
        ...
        r"\begin{table}[t]",
        r"\centering",
        r"\caption{\vd{R1.5/R2.6: Configuration regimes and execution accounting by blockchain and validator-set size ($V$). Config. reports $C$/A and M/N, denoting total, all-positive, mixed positive/non-positive, and no-positive configurations. Exec. reports O/S/P and Z/N, denoting observed, submitted, positive-commit, zero-commit, and no-submission executions. Valid T/L/$E^+$ gives point-valued TPS, latency, and positive-service energy observations.}}",
        ...
        r"\toprule",
        r"\textbf{Blockchain} & $\mathbf{V}$ & \shortstack{\textbf{Config.}\\$\mathbf{C/A}$\\$\mathbf{M/N}$} & \shortstack{\textbf{Exec.}\\$\mathbf{O/S/P}$\\$\mathbf{Z/N}$} & \shortstack{\textbf{Valid}\\$\mathbf{T/L/E^+}$} \\",
        r"\midrule",
    ]
    for row_index, (_, item) in enumerate(grouped.iterrows()):
        ...
        lines.append(
            ...
            f"\\shortstack{{{int(item['configurations'])}/"
            f"{int(item['only_positive'])}\\\\{int(item['mixed'])}/"
            f"{int(item['no_positive'])}}} & "
            f"\\shortstack{{{int(item['observed'])}/{int(item['submitted'])}/"
            f"{int(item['positive'])}\\\\{int(item['zero'])}/"
            f"{int(item['no_submission'])}}} & "
            f"{int(item['tps_point'])}/{int(item['latency'])}/"
            f"{int(item['energy_positive'])} \\\\"
        )
    lines.extend(
        [
            r"\midrule",
            r"\textbf{Total} & -- & "
            f"\\shortstack{{{int(totals['configurations'])}/"
            f"{int(totals['only_positive'])}\\\\{int(totals['mixed'])}/"
            f"{int(totals['no_positive'])}}} & "
            f"\\shortstack{{{int(totals['observed'])}/{int(totals['submitted'])}/"
            f"{int(totals['positive'])}\\\\{int(totals['zero'])}/"
            f"{int(totals['no_submission'])}}} & "
            f"{int(totals['tps_point'])}/{int(totals['latency'])}/"
            f"{int(totals['energy_positive'])} \\\\",
            r"\bottomrule",
            r"\end{tabular}",
            r"\end{table}",
            "",
