...
def render_outcome_table(counts: pd.DataFrame) -> str:
    ...
    lines = [
        ...
        r"\begin{table}[t]",
        r"\centering",
        r"\caption{\vd{R1.5/R2.6: Configuration regimes and execution accounting by blockchain and validator-set size. Panel~(a) reports $C$ and configurations with all-positive, mixed positive/non-positive, or no-positive outcomes. Panel~(b) reports observed/submitted executions, positive-commit/zero-commit/no-submission outcomes, and point-valued TPS/latency/positive-service energy observations, in the displayed order.}}",
        ...
        r"\toprule",
        r"\multicolumn{6}{@{}l}{\textbf{(a) Configuration regimes}} \\",
        r"\textbf{Blockchain} & \textbf{V} & $\mathbf{C}$ & \textbf{All} & \textbf{Mix} & \textbf{None} \\",
        r"\midrule",
    ]
    ...
    lines.extend(
        [
            ...
            r"\toprule",
            r"\multicolumn{5}{@{}l}{\textbf{(b) Execution outcomes and metric validity}} \\",
            r"\textbf{Blockchain} & \textbf{V} & \textbf{Obs./Sub.} & \shortstack{\textbf{Pos./Zero/}\\\textbf{No-sub.}} & \shortstack{\textbf{TPS/Lat.}\\$\mathbf{E^+}$} \\",
            r"\midrule",
        ]
    )
    for row_index, (_, item) in enumerate(grouped.iterrows()):
    ...
    lines.extend(
        [
            r"\midrule",
            r"\textbf{Total} & -- & "
            f"{int(totals['observed'])}/{int(totals['submitted'])} & "
