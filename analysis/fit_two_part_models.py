...
partial eta squared for the linear component, Type-III HC3 and
configuration-cluster Type-II sensitivity tests, residual diagnostics, and the
estimability of the two targeted three-way interactions requested during
review.
"""
...
# Omit run-time PDF timestamps so identical inputs produce byte-identical plots.
...
def configuration_cluster_type_ii_sensitivity(
    data: pd.DataFrame, metric: str
) -> pd.DataFrame:
    """Repeat the conditional Type-II tests with configuration-cluster covariance.

    HC3 is the primary heteroscedasticity-robust analysis. This sensitivity
    allows arbitrary residual dependence among repeated executions from the
    same blockchain-topology-workload-size configuration.
    """

    value_column, eligibility_column = METRIC_SPECS[metric]
    eligible = data.loc[data[eligibility_column]].copy()
    eligible["log_value"] = np.log(pd.to_numeric(eligible[value_column]))
    response, design, terms, diagnostics = estimable_design(
        f"log_value ~ {TWO_WAY_RHS}", eligible
    )
    model = sm.OLS(response.iloc[:, 0], design).fit()
    configuration_id = eligible[CONFIGURATION_COLUMNS].astype(str).agg("|".join, axis=1)
    clusters, unique_clusters = pd.factorize(configuration_id, sort=True)
    ...
    cluster_cov = np.asarray(clustered.cov_params())
    identity = np.eye(model.model.exog.shape[1])
    model_terms_only = [term for term in terms if term["name"] != "Intercept"]
    rows: list[dict[str, object]] = []
    for term in model_terms_only:
        name = str(term["name"])
        term_factors = set(term["factors"])
        own_columns = list(term["columns"])
        higher_columns: list[int] = []
        for other in model_terms_only:
            if term_factors < set(other["factors"]):
                higher_columns.extend(other["columns"])
        ...
        L2 = identity[higher_columns]
        if L2.size:
            lvl = L1 @ cluster_cov @ L2.T
            orthogonal, _ = linalg.qr(lvl, mode="full")
            numerator_df = len(own_columns)
            restriction = orthogonal[:, -numerator_df:].T @ L1
        else:
            restriction = L1
            numerator_df = len(own_columns)
        test = clustered.f_test(restriction)
        rows.append(
            {
                "term": TERM_DISPLAY.get(name, name),
                "term_formula": name,
                "test": "configuration-cluster Type-II F sensitivity",
                "statistic": float(test.fvalue),
                "df": float(numerator_df),
                "nominal_df": float(term["nominal_columns"]),
                "residual_df": float(test.df_denom),
                "p_value": float(test.pvalue),
                "configuration_clusters": int(len(unique_clusters)),
                "support_adjusted": bool(diagnostics["support_adjusted"]),
            }
        )
...
def render_term_table(
    ...
    lines = [
        ...
        r"Term & $\chi^2$ (df) & $p$ & $F_{\mathrm{HC3}}$ (df) & $p$ & $\eta_p^2$ & $F_{\mathrm{HC3}}$ (df) & $p$ & $\eta_p^2$ & $F_{\mathrm{HC3}}$ (df) & $p$ & $\eta_p^2$ \\",
        r"\hline",
    ]
    for _, row in outcome.iterrows():
        pieces = [
            term_labels.get(str(row["term"]), str(row["term"])),
            f"{row['statistic']:.2f} ({int(row['df'])})",
            ...
            if not bool(match.get("design_full_rank", True)):
                ...
                pieces.extend(
                    [
                        ...
                        f"({int(match['df'])},{int(match['residual_df'])})",
...
def render_three_way_sensitivity_table(
    ...
    lines = [
        r"\begin{table}[t]",
        r"\centering",
        r"\caption{\vd[R1.11]{Targeted three-way sensitivities: outcome rows use likelihood-ratio $\chi^2$ deletion tests and conditional-performance rows HC3 Type-II $F$ tests. For sparse positive-service support, df gives supported rank/nominal columns with no imputation; outcome tests are omnibus screening under quasi-separation.}}",
...
def run_models(input_path: Path, output_dir: Path, table_dir: Path) -> dict[str, object]:
    ...
    for scope in ("legacy_three_workloads", "six_workloads"):
        ...
        for metric in METRIC_SPECS:
            ...
            cluster_sensitivity = configuration_cluster_type_ii_sensitivity(
                data, metric
            )
            cluster_sensitivity.insert(0, "metric", metric)
            cluster_sensitivity_parts.append(cluster_sensitivity)
        ...
        cluster_sensitivity = pd.concat(
            cluster_sensitivity_parts, ignore_index=True
        )
        ...
        cluster_sensitivity.to_csv(
            scope_output / "performance_type_ii_configuration_cluster_sensitivity.csv",
            index=False,
        )
