...
BOOTSTRAP_STATISTICS = (
    "icc",
    "between_configuration_variance",
    "within_configuration_variance",
)
...
def cluster_bootstrap(
    ...
    for repetition in range(repetitions):
        ...
        try:
            ...
            finite = all(np.isfinite(estimate[key]) for key in BOOTSTRAP_STATISTICS)
            if estimate["converged"] and finite:
                for statistic in BOOTSTRAP_STATISTICS:
                    estimates[statistic][repetition] = float(estimate[statistic])
            else:
                failures += 1
...
def bootstrap_job(args: tuple[object, ...]) -> dict[str, object]:
    ...
    finite = estimates["icc"][np.isfinite(estimates["icc"])]
    ...
    for statistic, bootstrap_values in estimates.items():
        finite_values = bootstrap_values[np.isfinite(bootstrap_values)]
        intervals[f"{statistic}_ci_lower"] = float(np.quantile(finite_values, 0.025))
        intervals[f"{statistic}_ci_upper"] = float(np.quantile(finite_values, 0.975))
    return {
        "scope": scope,
        ...
        **point,
        # Retain the original names for compatibility with existing consumers.
        "ci_lower": intervals["icc_ci_lower"],
        "ci_upper": intervals["icc_ci_upper"],
        **intervals,
...
def render_icc_table(table: pd.DataFrame, repetitions: int) -> str:
    lines = [
        r"\begin{table}[t]",
        r"\centering",
        rf"\caption{{\vd[R1.9/R2.4]{{Within-blockchain REML random-intercept decomposition of log positive-service observations. $C/N$ denotes metric-eligible configurations/executions; $\hat{{\tau}}^2/\hat{{\sigma}}^2$ are between-/within-configuration variances. Brackets give 95\% intervals from {repetitions:,} configuration-cluster bootstrap replicates.}}}}",
        ...
        r"\toprule",
        r"Blockchain & TPS & Latency & Energy \\",
        r"\midrule",
    ]
    for blockchain in sorted(table["blockchain"].unique()):
        ...
        for metric in METRIC_SPECS:
            ...
            cells.append(
                r"\shortstack{"
                f"$C/N={int(row['n_configurations'])}/{int(row['n_observations'])}$"
                r"\\"
                f"$\\hat{{\\tau}}^2={row['between_configuration_variance']:.3f}$ "
                f"[{row['between_configuration_variance_ci_lower']:.3f},"
                f"{row['between_configuration_variance_ci_upper']:.3f}]"
                r"\\"
                f"$\\hat{{\\sigma}}^2={row['within_configuration_variance']:.3f}$ "
                f"[{row['within_configuration_variance_ci_lower']:.3f},"
                f"{row['within_configuration_variance_ci_upper']:.3f}]"
                r"\\"
                f"ICC={row['icc']:.3f} "
                f"[{row['icc_ci_lower']:.3f},{row['icc_ci_upper']:.3f}]"
                r"}"
            )
...
def render_tables_from_results(output_dir: Path, table_dir: Path) -> dict[str, object]:
    ...
    required = {
        ...
        "n_configurations",
        "between_configuration_variance",
        "between_configuration_variance_ci_lower",
        "between_configuration_variance_ci_upper",
        "within_configuration_variance",
        "within_configuration_variance_ci_lower",
        "within_configuration_variance_ci_upper",
        "icc",
        "icc_ci_lower",
        "icc_ci_upper",
