...
from analysis.fit_two_part_models import (
    METRIC_SPECS,
    configuration_cluster_type_ii_sensitivity,
    fit_binomial,
    fit_linear_metric,
...
class TwoPartModelTests(unittest.TestCase):
    ...
    def test_generated_table_has_stable_label(self):
        ...
        self.assertIn(r"\caption{\vd[R1.9-10/R2.4]{", latex)
        ...
        self.assertIn("main-factor rows are hierarchical omnibus tests", latex)
        self.assertIn(r"$\chi^2$ (df)", latex)
        self.assertIn(r"$F_{\mathrm{HC3}}$ (df)", latex)
    ...
    def test_configuration_cluster_sensitivity_uses_configuration_groups(self):
        table = configuration_cluster_type_ii_sensitivity(self.six, "TPS")
        self.assertEqual(len(table), 10)
        self.assertTrue(table["statistic"].notna().all())
        self.assertTrue(table["p_value"].between(0, 1).all())
        self.assertEqual(table["configuration_clusters"].nunique(), 1)
        eligible = self.six.loc[self.six[METRIC_SPECS["TPS"][1]]]
        expected_clusters = eligible.groupby(
            CONFIGURATION_COLUMNS,
            observed=False,
        ).ngroups
        self.assertEqual(
            int(table["configuration_clusters"].iloc[0]), expected_clusters
        )
        self.assertTrue((table["residual_df"] == expected_clusters - 1).all())
