...
All selected measurements have `dynamic=0`; the generator instantiates static
link parameters from the trace, without a time-varying topology schedule. The
mode names identify these generator-defined instances and should not be read as
guarantees of canonical graph regularity. The checksums verify regeneration of
the documented reference instances, while the absence of a historical
topology manifest precludes a cryptographic binding to each released run row.

## Validation

The complete revision workflow was validated on 6 September 2026 with Python
3.12 and the pinned dependencies in `requirements.txt`. Its tracked products
include sixteen standalone LaTeX tables across the primary and compatibility
scopes and nine PDF figures (three run-level deviation figures and six model
diagnostics).

The legacy provenance script was last validated on 29 July 2026. It produced
one 300-row, 50-column aggregate CSV, six LaTeX tables, and twelve figure
variants in both PNG and PDF.
