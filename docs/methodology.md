...
All selected rows have `dynamic=0`: a static, trace-derived set of link
parameters is instantiated for each run, and no time-varying topology schedule
is applied. The topology labels denote the generator modes used to create the
frozen instances, not canonical mathematical graphs. Structural comparisons
therefore use the measured properties of those instances rather than assuming
regularity or connectivity from their names. The regenerated XML checksums
bind the reference instances to the documented generator and inputs; because
the released run rows contain no historical topology manifest, they do not by
themselves establish a cryptographic per-run binding.
...
are retained as a sensitivity. A second Type-II sensitivity uses
configuration-cluster covariance, allowing arbitrary residual dependence among
executions from the same blockchain--topology--workload--size cell. The cluster
sensitivity uses the finite-sample covariance correction and $G-1$ denominator
degrees of freedom, where $G$ is the number of eligible configurations. HC3
alone does not model that within-cell dependence. Residual-versus-fitted and
Q--Q plots, Breusch--Pagan statistics, and Jarque--Bera statistics are written
with the machine-readable term tables.
...
sensitivities, configuration-cluster Type-II sensitivities, targeted three-way
sensitivity tests, design-rank audits, residual diagnostics, and generated model
